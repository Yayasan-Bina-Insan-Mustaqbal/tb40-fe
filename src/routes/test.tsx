...
import { saveUser, saveAnswer, saveResult, getUserData, saveTierAnswers } from '@/lib/analytics'
...
function TestWizard() {
  ...
  // 1. Check user metadata on load (and handle resume)
  useEffect(() => {
    const handleResume = async () => {
      if (resume) {
        setIsLoading(true)
        try {
          const res = await getUserData({ data: resume })
          if (res.success && res.session) {
            // Reconstruct userMetadata
            const restoredMetadata = {
              ...
              orgName: res.session.org_id ? 'Restored Org' : '', // We don't have org name here easily without join, but it's fine
              apiType: 'live',
              apiUrl: 'https://tb40.insanmustaqbal.or.id',
            }
            
            localStorage.setItem("tb40_umum", JSON.stringify(restoredMetadata))
            setUserMetadata(restoredMetadata)
            setTestMode(restoredMetadata.testMode as any)
            
            // Set session
            updateSession({ id: resume })
            
            // Reconstruct answers based on mode
            if (res.session.test_mode === 'adaptive' && res.session.parsedAnswers) {
              localStorage.setItem("tb40_answers_v2", JSON.stringify(res.session.parsedAnswers))
              setAnswersV2(res.session.parsedAnswers)
              // Restore tier
              if (res.session.parsedAnswers.tier_2 && res.session.parsedAnswers.tier_2.length === 3) {
                 if (res.session.parsedAnswers.tier_3) setCurrentTier("tier_3")
                 else setCurrentTier("tier_3")
              } else if (res.session.parsedAnswers.tier_1) {
                 setCurrentTier("tier_2")
              }
            } else if (res.session.parsedAnswers) {
              // Convert object { "1": 60 } to array
              const arr = new Array(40).fill(60)
              Object.entries(res.session.parsedAnswers).forEach(([k, v]) => {
                arr[parseInt(k) - 1] = v as number
              })
              localStorage.setItem("tb40_answers", JSON.stringify(arr))
              setAnswers(arr)
            }
            
            // Remove ?resume from URL
      ...
      try {
        ...
        if (!savedMetadata) {
          console.log("DEBUG: savedMetadata is null!")
          navigate({ to: "/" })
          return
        }
        ...
        console.log("DEBUG: Error in test.tsx init:", e)
        console.error(e)
        navigate({ to: "/" })
      }
    }
  ...
  // Initialize session for analytics tracking
  useEffect(() => {
    ...
    // Save user to analytics database
      ...
      try {
        const result = await saveUser({
          data: {
            ...
            nickName: userMetadata.nama?.panggilan,
            orgName: userMetadata.orgName,
        ...
        if (result.success && result.userId) {
          updateSession({ userId: result.userId })
          console.log('Analytics session initialized:', session!.id)
        }
      } catch (error) {
    ...
    async (type: string, currentAnswersV2: any) => {
      ...
      try {
        ...
        try {
          const response = await fetch("/questions.json")
          const data = await response.json()
          console.log("DEBUG: Fetched /questions.json fallback", data)
          qList =
            data.parts?.[type === "tb40anak" ? "tb40anak" : "tb40Dewasa"]
              ?.questions || []
          console.log("DEBUG: qList length after fallback:", qList.length, "Type:", type)
        } catch (e) {
  ...
  const handleTier1Answer = (introvert: number, extrovert: number) => {
    ...
    if (session?.userId) {
      ...
      }).catch(err => console.error("Failed to save tier answers", err))
    }
  }
  ...
  // Handle tier_3 slider change — key by original question index (with auto-save)
  const handleV2Tier3Change = (qIndex: number, val: number) => {
    ...
    // Auto-save to analytics (fire-and-forget)
    const session = getSession()
    if (session?.userId) {
      saveAnswer({
        data: {
          ...
          questionId: qIndex, // Already 1-indexed
          answerValue: val,
        }
  ...
  // Save V1 answer (with auto-save to analytics)
  const handleAnswerChange = async (questionIndex: number, val: number) => {
    ...
    if (session?.userId) {
      saveAnswer({
        data: {
          sessionId: session.id,
          userId: session.userId,
          questionId: questionIndex + 1, // Convert to 1-indexed
          answerValue: val,
        }
      }).catch(err => console.error('Failed to save answer:', err))
    }
  ...
  const submitTest = async () => {
    ...
    try {
      ...
      if (session?.userId && resultData.parts?.[type]) {
        const scores = resultData.parts[type]
        await saveResult({
          data: {
            sessionId: session.id,
            userId: session.userId,
            rawScores: scores,
            percentileScores: scores, // Same for now
        ...
        <div className="flex min-h-screen flex-col bg-background p-4 text-foreground md:p-8">
          <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
            ...
            <div className="mt-2 flex items-center justify-between border-b border-border pb-4">
              ...
              >
                <RotateCcw className="h-3 w-3" /> Batalkan Tes
              </button>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col gap-1.5 text-left">
                  <DialogTitle className="font-heading text-lg font-semibold">
                    Batalkan Tes & Hapus Data?
                  </DialogTitle>
                  <DialogDescription className="text-sm leading-relaxed">
                    Apakah Anda yakin ingin membatalkan tes? Tindakan
