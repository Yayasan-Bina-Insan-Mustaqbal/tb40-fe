...
test.describe('TB40 Frontend E2E - Adaptive Assessment (v0.2)', () => {
  test('should complete the entire adaptive assessment flow successfully', async ({ page }) => {
    ...
    await expect(page.locator('text=Visualisasi Pemetaan Tafsir Bakat')).toBeVisible();
    await expect(page.locator('text=Grafik Data Interaktif')).toBeVisible();

    // Log LocalStorage values to console
    const savedUmum = await page.evaluate(() => localStorage.getItem("tb40_umum"));
    const savedAnswersV1 = await page.evaluate(() => localStorage.getItem("tb40_answers"));
    const savedAnswersV2 = await page.evaluate(() => localStorage.getItem("tb40_answers_v2_tier3"));
    console.log("DEBUG - tb40_umum in localStorage:", savedUmum);
    console.log("DEBUG - tb40_answers in localStorage:", savedAnswersV1);
    console.log("DEBUG - tb40_answers_v2_tier3 in localStorage:", savedAnswersV2);
    ...
    await expect(page.locator('h3:has-text("Bagikan Hasil Penilaian")')).toBeVisible();
